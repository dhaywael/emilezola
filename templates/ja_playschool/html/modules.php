...
function modChrome_T3Section($module, &$params, &$attribs) { 
	...
	if($params->get('color-module')) {
		$bgModule = 'background-color:'.$params->get('color-module').';';
	}
	...
	if($params->get('color-module') || $params->get('color-module')) {
		$modStyle = 'style="'.$bgModule.' '.$bgimageModule.'"';
	}
	...
	if($params->get('pattern-top')) {
		$patternTop = '<div class="pattern pattern-top" style="background-image:url('.JUri::root().''.$params->get('pattern-top').');height:'.$params->get('pattern-height-top').';"></div>';
	}

	// Pattern Bottom
	$patternBottom = '';

	if($params->get('pattern-bottom')) {
		$patternBottom = '<div class="pattern pattern-bottom" style="background-image:url('.JUri::root().''.$params->get('pattern-bottom').');height:'.$params->get('pattern-height-bottom').';"></div>';
	}

	if (!empty ($module->content)) {
		$html = "<{$moduleTag} class=\"t3-module module{$moduleClassSfx} {$moduleClass} {$paddingBottom} {$paddingTop}\" id=\"Mod{$module->id}\"  {$modStyle} >{$patternTop}" .
					"<div class=\"{$moduleWidth}\"><div class=\"module-inner\">";

		if (($module->showtitle != 0) && ($moduleType != 'mod_ja_acm')) {
			$html .= "<{$headerTag} class=\"module-title {$headerClass}\" {$colorTitle}><span class=\"sub-heading\"><span {$styleSub}>{$moduleSub}</span></span>{$module->title}</{$headerTag}>";
		}

		$html .= "<div class=\"module-ct\">{$module->content}</div></div></div>{$patternBottom}</{$moduleTag}>";
