...
class PlgAjaxJacontenttype extends JPlugin
{
	...
	protected function  getMediaList() {
		...
		$docs = method_exists( $model, 'getVideos') ? array_merge( $model->getDocuments(), $model->getVideos() ) : $model->getDocuments();
		$images = $model->getDocuments();
	...
	protected function  getContacts() {
		...
		if (version_compare(JVERSION, '4.0', 'ge'))
			$model = new Joomla\Component\Contact\Administrator\Model\ContactsModel(array()); 
		else
